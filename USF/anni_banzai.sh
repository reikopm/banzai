...
cp /home/mbonteam/USF/Anni/scripts/anni_banzai.sh "${ANALYSIS_DIR}"/analysis_script.txt
...
for CURRENT_LIB in $LIBRARY_DIRECTORIES; do
  ...
  echo $(date +%H:%M) "PEAR output compressed."
	...
	if [ "${RENAME_READS}" = "YES" ]; then
		...
		awk -F'[: ]' '{
				if ( /^>/ )
					print ">"$4":"$5":"$6":"$7"_lib_'${CURRENT_LIB##*/}'_tag_";
				else
					print $0
...
################################################################################
	...
	for TAG_SEQ in $TAGS; do
		...
		# BLAST CLUSTERS
		blastn -query "${BLAST_INPUT}" -db "$BLAST_DB" -num_threads "$n_cores" -perc_identity "${PERCENT_IDENTITY}" -word_size "${WORD_SIZE}" -evalue "${EVALUE}" -max_target_seqs "${MAXIMUM_MATCHES}" -outfmt 5 -out "${TAG_DIR}"/anni_10_BLASTed.xml
	done
fi
...
for DIR in "$DIRECTORIES"; do
	...
	# PERFORM COMMON ANCESTOR GROUPING IN MEGAN
		...
		BLAST_XML="${DIR}"/anni_10_BLASTed.xml
...
lcapercent=${LCA_PERCENT};" > "${MEGAN_COMMAND_FILE}"
		...
		if [ "$PERFORM_SECONDARY_MEGAN" = "YES" ]; then
			...
			echo "update;" >> "${MEGAN_COMMAND_FILE}"
			echo "select nodes=all;" >> "${MEGAN_COMMAND_FILE}"
			echo "export what=CSV format=readname_taxonname separator=comma file='${DIR}/meganout_${COLLAPSE_RANK3}.csv';" >> "${MEGAN_COMMAND_FILE}"
